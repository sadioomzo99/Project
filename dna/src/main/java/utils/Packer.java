...
public class Packer {
    ...
    public enum Type {
        ...
        public static final int MIN_SIG_LEN = Arrays.stream(Packer.Type.values()).mapToInt(t -> t.signature.length()).min().orElseThrow();
        public static final int MAX_SIG_LEN = Arrays.stream(Packer.Type.values()).mapToInt(t -> t.signature.length()).max().orElseThrow();
         ...
         */
        public static Packer.Type parsePrefix(BitString bs) {
