...
public class PooledCompletionService<T> extends ExecutorCompletionService<T> implements Streamable<T> {
    ...
    public Future<T> submitAndShutDown(Callable<T> task) {
        return submit(() -> {
            T called = task.call();
            shutdown();
            return called;
    ...
    public void submitAndShutDown(Runnable task, T result) {
        submit(() -> {
            task.run();
            shutdown();
        }, result);
    ...
    public void waitFinish() {
        FuncUtils.safeCall(() -> this.pool.awaitTermination(Integer.MAX_VALUE, TimeUnit.DAYS));
    }
