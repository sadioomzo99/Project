...
public class BaseSequence implements Streamable<Base>, Cloneable {
    ...
    /**
     * Creates a new DNA sequence of the Base list provided with its properties.
     * @param bases the list of Base that will be copied into this instance.
     * @param properties the properties for this BaseSequence's instance.
     */
    public BaseSequence(List<Base> bases, HashMap<String, Object> properties) {
    ...
    /**
     * Gets the property's value if present. If the property's value is not present, the new value is computed, put to this instance, and returned.
     * @param propertyName the property name.
     * @param valueSupp the property's value supplier that is used in case the property is not present.
     */
    public <T> T getOrPutProperty(String propertyName, Supplier<T> valueSupp) {
        if (properties == null)
            properties = new HashMap<>();

        return (T) properties.computeIfAbsent(propertyName, __ -> valueSupp.get());
     ...
     */
    public boolean contains(BaseSequence seq) { return Collections.indexOfSubList(this.bases, seq.bases) >= 0;
    }
    ...
    public BaseSequence clone() {
        if (properties == null)
            return new BaseSequence(this.bases, null);

        return new BaseSequence(this.bases, (HashMap<String, Object>) properties.clone());
    }
