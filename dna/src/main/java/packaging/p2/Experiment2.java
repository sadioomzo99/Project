...
public class Experiment2 {
    public static void main(String[] args) {
        ...
        System.out.println("----------------- finished Application on params_seg.ini in: " + FuncUtils.asPrettyString(Duration.between(applicationSegStart, applicationNoSegStart)));

        System.out.println("----------------- running Application on params_noseg.ini");
        Application.main("params_noseg.ini", "true");
        System.out.println("----------------- finished Application on params_noseg.ini in: " + FuncUtils.asPrettyString(Duration.between(applicationNoSegStart, Instant.now())));
        ...
        eval(true,
                MinHashLSH.newSeqAmpLSHTraditional(4, 200, 20, LSHStorage.AmplifiedLSHStorage.Amplification.OR),
        ...
        System.out.println("----------------- the evaluation of the sequences with segmentation took: " + FuncUtils.asPrettyString(Duration.between(evalSegStart, evalNoSegStart)));
        ...
        System.out.println("----------------- the evaluation of the sequences without segmentation took: " + FuncUtils.asPrettyString(Duration.between(evalNoSegStart, end)));
        System.out.println("----------------- the whole evaluation took: " + FuncUtils.asPrettyString(Duration.between(evalSegStart, end)));
        System.out.println("----------------- everything took: " + FuncUtils.asPrettyString(Duration.between(start, end)));
        System.out.println("----------------- done");
    }

    static Aggregator.NumberAggregates distAggregate(List<BaseSequence> seqs, LSH<BaseSequence> lsh) {
        ...
        return Aggregator.aggregateNumbers(seqs.stream().parallel()
                .mapToDouble(seq -> Math.min(DistanceCoder.distanceScoreExclusive(seq, lsh), DistanceCoder.distanceScore(seq.complement(), lsh))).toArray(), true);
    }
    ...
    static Aggregator.NumberAggregates[] aggregate(List<BaseSequence> seqs, LSH<BaseSequence> lsh, DNARule rule) {
    ...
    static void eval(boolean segmentation, MinHashLSH<BaseSequence, ?> cbbsLSH, MinHashLSH<BaseSequence, ?> infoDnaLSH, MinHashLSH<BaseSequence, ?> oligosLSH, String... filePaths) {
