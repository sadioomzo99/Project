...
public class Experiment1 {
    ...
    public static void main(String[] args) {
        ...
        for (var dnaConvertor : dnaConvertors) {
            ...
            for (int targetLength : targetLengths) {
                ...
                for (int numPermutations : numsPermutations) {
                    ...
                    var lsh = MinHashLSH.newSeqAmpLSHTraditional(4, 200, 20, LSHStorage.AmplifiedLSHStorage.Amplification.OR);
        ...
        System.out.println("\n-----------------------------------------------------------\nExperiment 1 took: " + FuncUtils.asPrettyString(Duration.between(start, Instant.now())) + "\n-----------------------------------------------------------\n");
    }


    public static Aggregator.NumberAggregates distStats(List<BaseSequence> seqs, LSH<BaseSequence> lsh) {
        return Aggregator.aggregateNumbers(seqs.stream().parallel().mapToDouble(seq -> Math.min(DistanceCoder.distanceScore(seq, lsh), DistanceCoder.distanceScore(seq.complement(), lsh))).toArray(), true);
    }
}
