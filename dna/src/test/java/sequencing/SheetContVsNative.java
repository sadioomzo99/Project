...
// TODO
public class SheetContVsNative {
    ...
    public static void main(String[] args) {
        ...
        BufferedCsvWriter writer = new BufferedCsvWriter("D:/Promotion/paper_4/experimentations/corrected_c.csv", false);
        ...
        for (boolean constantNumTuples : Arrays.asList(true)) {
            for (var selectivity : Arrays.asList(0.0d, 0.1d, 0.2d, 0.3d, 0.4d, 0.5d, 0.6d, 0.7d, 0.8d, 0.9d, 1.0d)) {
                for (var internalPlat : allMatches.entrySet()) {
                    ...
                    for(var leafPlat : allMatches.entrySet()) {
                        ...
                        for (int b : List.of(512)) {
                            long internalNodeSize = 2L * b - 1L;
                            for (int c : List.of(512)) {
                                for (int h = 1; h < 10; h++) {
                                    long leafSize = 2L * c - 1L;

                                    double numberTuples = 1_000_000_000d;
                                    double numLeafNodes = Math.ceil(numberTuples / leafSize);


                                    double numInternalNodes = Commons.numInternals(b, h);
