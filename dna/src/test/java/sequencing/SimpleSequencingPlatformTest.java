...
public class SimpleSequencingPlatformTest {

    public static void main(String[] args) {

        BufferedCsvWriter writer = new BufferedCsvWriter("D:/Promotion/paper_4/results/new/sequencing_analysis_simple.csv", false);
        ...
        for (double bitrate : List.of(2d, 1.5d, 1.0d, 0.5d)) {
                ...
                for (int c : List.of(2, 16, 64, 128, 256, 512)) {
                    ...
                    for (long numberTuples : List.of(10_000_000L)) {
                        for (long tupleSize : List.of(100L)) {
                            for (double selectivity : List.of(0.0d, 0.01d, 0.1d, 0.2d, 0.5d, 0.9d, 0.99d, 0.999d)) {
                                    ...
                                    for (var bufferedLeafNodes : DoubleStream.of(0d).mapToObj(factor -> new Pair<>(Math.ceil(factor * numLeafNodes), factor)).toList()) {
                                        ...
                                        for (SimpleSequencingPlatform platformInternalNodes : Commons.SIMPLE_PLATFORMS) {
                                            ...
                                            for (SimpleSequencingPlatform platformLeafNodes : Commons.SIMPLE_PLATFORMS) {
                                                ...
                                                //double totalCostReadAllNGS = sequencingCostInternalNodes.average().doubleValue() + sequencingCostLeafNodes.average().doubleValue();
                                        ...
                                        Commons.SequencingCombo bestForQuery = combis.stream().min(Comparator.comparingDouble(Commons.SequencingCombo::querySequencingCost)).orElseThrow();
                                        Commons.SequencingCombo bestForSequenceAll = combis.stream().min(Comparator.comparingDouble(Commons.SequencingCombo::totalCostReadAll)).orElseThrow();
