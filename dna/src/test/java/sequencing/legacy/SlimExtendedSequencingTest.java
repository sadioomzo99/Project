...
public class SlimExtendedSequencingTest {

    public static void main(String[] args) {
        BufferedCsvWriter writer = new BufferedCsvWriter("D:/Promotion/paper_4/results/new/sequencing_analysis_slim_extended_real_br.csv", false);
        ...
        for (Bitrate bitrate : List.of(
                new Bitrate(70, 1.8d, 0.721151d, 0.6421171),
                new Bitrate(250, 1.8d, 1.1082608d, 0.9930812d),
                new Bitrate(1_000, 1.8d, 1.5380145d, 1.3836005d),
                new Bitrate(10_000, 1.8d, 1.5985689d, 1.4322257d),
                new Bitrate(15_000, 1.8d, 1.5545664d, 1.4150927d),
                new Bitrate(20_000, 1.8d, 1.4857032d, 1.3947997d),
                new Bitrate(60_000, 1.8d, 1.3974718d, 1.2620766d)
        )) {
            for (int b : List.of(64)) {
                int internalNodePointers = 2 * b - 1;
                for (int c : List.of(64)) {
                    int leafSize = 2 * c - 1;
                    for (long numberTuples : List.of(100_000_000L)) {
                        for (long tupleSize : List.of(1000L)) {
                                    ...
                                    for (var bufferedLeafNodes : DoubleStream.of(0d).mapToObj(factor -> new Pair<>(Math.ceil(factor * numLeafNodes), factor)).toList()) {
                                        ...
                                        for (var plat : Commons.SIMPLE_PLATFORMS) {
                                            write(
                                                    ...
                                                    plat.generation(),
                                                    0.0d,
                                                    plat.sequence(objectsAsGigaNucleotidesRaw).average().doubleValue()
                                            );
                                        }
                                        ...
                                        for (SimpleSequencingPlatform platformInternalNodes : Commons.SIMPLE_PLATFORMS) {
                                            ...
                                            for (SimpleSequencingPlatform platformLeafNodes : Commons.SIMPLE_PLATFORMS) {
                                                ...
                                                write(
                                                        ...
                                                        platformInternalNodes.generation(),
                                                        platformLeafNodes.generation(),
                                                        queryEvaluationCost.visitedInternalNodes() * sequencingCostPerInternalNode,
                                                        queryEvaluationCost.visitedLeafNodes() * sequencingCostPerLeafNode
                                                );
                                            }
                                        }
